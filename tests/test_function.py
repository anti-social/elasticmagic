...
def test_weight():
    assert Weight(3).to_elastic(Compiler_5_0) == {"weight": 3}
...
def test_factor():
    assert Factor(PostDocument.popularity).to_elastic(Compiler_5_0) == {
        "field_value_factor": {
            "field": "popularity"
        }
    }
...
def test_random_score():
    assert RandomScore(17).to_elastic(Compiler_5_0) == {"random_score": {"seed": 17}}


def test_gauss():
    assert Gauss(PostDocument.created_date, origin='now', scale='1h').to_elastic(Compiler_5_0) == {
        "gauss": {
...
def test_exp():
    assert Exp(PostDocument.popularity, origin=0, scale=20).to_elastic(Compiler_5_0) == {
        "exp": {
